...
export function DashboardHome() {
  ...
  const QUICK_STATS = [
    { label: "Today Revenue", value: loading ? "..." : summary?.today_revenue || 0, delta: "+8.2%", color: "#F59E0B", icon: <TrendingUp size={16} /> },
  ...
  const MODULES = [
    ...
    { label: "Orders", path: "/dashboard/orders", icon: <ShoppingCart size={20} />, color: "#3B82F6", desc: "Open and dispatched" },
  ...
  return (
    <div style={{ padding: "1.5rem", maxWidth: 1400 }}>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={{ marginBottom: "1.5rem" }}>
        ...
        <p style={{ color: "#8892a4", fontFamily: "Inter,sans-serif", fontSize: "0.9rem" }}>Here is your AYASTRA overview for today.</p>
      </motion.div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1rem", marginBottom: "1.5rem" }}>
        {QUICK_STATS.map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.07 }}
            style={{ background: "rgba(11,17,32,0.9)", border: "1px solid rgba(100,100,100,0.2)", borderRadius: 14, padding: "1.1rem 1.25rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              ...
              <div style={{ color: s.color, background: "rgba(100,100,100,0.1)", borderRadius: 8, padding: "0.4rem" }}>{s.icon}</div>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "1rem", marginBottom: "1.5rem" }}>
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
            <span style={{ color: "#e8eaf0", fontFamily: "Space Grotesk,sans-serif", fontWeight: 600 }}>Revenue This Week</span>
            <span style={{ color: "#22C55E", fontSize: "0.7rem", fontFamily: "monospace" }}>+18.4% vs last week</span>
          </div>
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={revenueData.length > 0 ? revenueData : [{ day: "Mon", revenue: 0 }]} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {alerts.length === 0 ? (
              <div style={{ color: "#4B5563", fontSize: "0.8rem" }}>No alerts yet</div>
            ) : alerts.map((a, i) => (
              <div key={i} style={{ display: "flex", gap: "0.5rem", alignItems: "flex-start" }}>
                ...
                <div>
                  <div style={{ color: "#e8eaf0", fontSize: "0.78rem" }}>{a.message}</div>
                  <div style={{ color: "#4B5563", fontSize: "0.68rem", marginTop: 1 }}>{a.created_at}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "0.75rem" }}>
          {MODULES.map((m) => (
            <motion.button key={m.label} onClick={() => navigate(m.path)} whileHover={{ scale: 1.03 }}
              ...
              <div style={{ color: m.color, background: "rgba(100,100,100,0.1)", borderRadius: 10, padding: "0.5rem", flexShrink: 0 }}>{m.icon}</div>
              <div>
                <div style={{ color: "#e8eaf0", fontFamily: "Space Grotesk,sans-serif", fontWeight: 600, fontSize: "0.85rem" }}>{m.label}</div>
                <div style={{ color: "#4B5563", fontSize: "0.7rem" }}>{m.desc}</div>
              </div>
            </motion.button>
          ))}
        </div>
